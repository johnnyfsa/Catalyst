...
namespace Catalyst.UI.Presentation.ReactionTable
{
    public sealed class TableActionButtonVisual : MonoBehaviour
    {
        ...
        [SerializeField] private VisualState previewState = VisualState.Inactive;
        ...
        {
            ApplyState(VisualState.Inactive);
        }

        public void SetActive()
        {
            ApplyState(VisualState.Active);
        }

        public void ApplyState(VisualState state)
        {
            CurrentState = state;

            bool isActive = state == VisualState.Active;

            if (activeOnlyObjects != null)
            {
                foreach (GameObject visual in activeOnlyObjects)
                {
